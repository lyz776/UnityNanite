...
float3 GIEvaluateTODDiffuseSky(float3 normal)
{
    ...
    // A compact cosine-hemisphere approximation.  Sampling the normal preserves
    // directional TOD colour while the vertical samples stabilize horizon/ground.
    float3 vertical = normal.y >= 0.0 ? float3(0.0, 1.0, 0.0) : float3(0.0, -1.0, 0.0);
    return GIEvaluateTODSky(normal) * 0.60 + GIEvaluateTODSky(vertical) * 0.40;
}
